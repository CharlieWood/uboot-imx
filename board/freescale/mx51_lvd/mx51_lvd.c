...
static struct fb_videomode chimei_wxga = {
	/* 1024x768 @ 52 Hz , pixel clk @ 44.333MHz / 22557 ps */
...
static void setup_lcd_io(void)
{
	u32 reg;

	/* pull LCD_PWR_ON to low, power off - DI1_PIN11 GPIO3_0 */
	...
	/* assert LCD_STBYB to low, standby mode - DI1_PIN12 GPIO3_1 */
	reg = readl(GPIO3_BASE_ADDR + 0x4);	/* GPIO3 GDIR */
	reg |= 0x00000002;  /* configure GPIO3_2 as output */
	writel(reg, GPIO3_BASE_ADDR + 0x4);
	reg = readl(GPIO3_BASE_ADDR + 0x0);	/* GPIO3 DR */
	reg &= ~0x00000002;  /* configure GPIO3_2 to low */
	writel(reg, GPIO3_BASE_ADDR + 0x0);
	mxc_request_iomux(MX51_PIN_DI1_PIN12, IOMUX_CONFIG_ALT4);

	/* assert LVDS_SHDN to low, shutdown mode - DISPB2_SER_RS GPIO_3_8 */
	reg = readl(GPIO3_BASE_ADDR + 0x4);
	reg |= 0x00000100;
	writel(reg, GPIO3_BASE_ADDR + 0x4);
	reg = readl(GPIO3_BASE_ADDR + 0x0);
...
static void board_sys_on_off_ctl(int on)
{
	...
	/* SYS_ON_OFF_CTL(GPIO1_3) pin control */
...
void lcd_enable(void)
{
	...
	/* LCD STBYB de-assert to high, active mode */
	reg = readl(GPIO3_BASE_ADDR + 0x0);
	reg |= 0x00000002;
	writel(reg, GPIO3_BASE_ADDR + 0x0);
	...
	ret = ipuv3_fb_init(&chimei_wxga, 0, IPU_PIX_FMT_RGB24,
			DI_PCLK_PLL3, 0);
	if (ret)
...
static void panel_info_init(void)
{
	...
	panel_info.vl_col = chimei_wxga.xres;
	panel_info.vl_row = chimei_wxga.yres;
...
#define USBCMD 0x140
	...
	gd->bd->bi_arch_number = MACH_TYPE_MX51_LVD;	/* board id for linux */
...
int board_late_init(void)
{
...
#ifdef CONFIG_LCD
	/* Enable LCD output & backlight later after draw logo on FB */
	...
	imx_pwm_config(pwm0, 8000, 50000);
	imx_pwm_enable(pwm0);
...
static void check_poweron(void)
{
	...
	/*LOW_POWER_ENABLE_CTL(GPIO1_6) pin control */
	mxc_request_iomux(MX51_PIN_GPIO1_6, IOMUX_CONFIG_ALT0);
	reg = readl(GPIO1_BASE_ADDR + 0x4);	/* GPIO1 GDIR */
	reg |= 0x00000040;  /* configure GPIO1_6 lines as output */
	writel(reg, GPIO1_BASE_ADDR + 0x4);
	reg = readl(GPIO1_BASE_ADDR + 0x0);	/* GPIO1_6 DR */
	reg &= ~0x00000040;  /* configure GPIO1_6 lines to low */
	writel(reg, GPIO1_BASE_ADDR + 0x0);

	/*26M_OSC_EN_CTL(GPIO1_5) pin control */
	mxc_request_iomux(MX51_PIN_GPIO1_5, IOMUX_CONFIG_ALT0);
	reg = readl(GPIO1_BASE_ADDR + 0x4);	/* GPIO1 GDIR */
	reg |= 0x00000020;  /* configure GPIO1_5 lines as output */
	writel(reg, GPIO1_BASE_ADDR + 0x4);
	reg = readl(GPIO1_BASE_ADDR + 0x0);	/* GPIO1_5 DR */
	reg |= 0x00000020;  /* configure GPIO1_5 lines to high */
	writel(reg, GPIO1_BASE_ADDR + 0x0);
