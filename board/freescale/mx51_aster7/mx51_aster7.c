...
static void board_sys_on_off_ctl(int on)
{
	...
	val |= 0x00800000;  /* configure GPIO1_23 lines as output */
	writel(val, GPIO1_BASE_ADDR + 0x4);
	val = readl(GPIO1_BASE_ADDR + 0x0);	/* GPIO1 DR */
	if (on)
		val |= 0x00800000;  /* configure GPIO1_23 lines to high */
	else
		val &= ~0x00800000;  /* configure GPIO1_23 lines to low */
	writel(val, GPIO1_BASE_ADDR + 0x0);
	mxc_request_iomux(MX51_PIN_UART3_TXD, IOMUX_CONFIG_ALT3);
...
static void check_battery(struct spi_slave *slave)
{
	...
	if (bat_vol_mV < 6800) {
		/* battery voltage lower than 6.8V */
		puts("Battery lower than 6.8V, shutdown, please charger battery!!!\n");
...
void lcd_enable(void)
{
	...
	mxc_request_iomux(MX51_PIN_NANDF_CS5, IOMUX_CONFIG_ALT3);
...
#define USBCMD 0x140
	...
	gd->bd->bi_arch_number = MACH_TYPE_MX51_ASTER7;	/* board id for linux */
...
#ifdef CONFIG_LCD
	/* Enable LCD backlight later after draw logo on FB */
